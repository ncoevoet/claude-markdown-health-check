...
| `depth` | `"auto"\|"quick"\|"standard"\|"deep"` | `"auto"` | Depth floor / override. `auto` runs Standard, upgrading to Deep for a large ecosystem (Phase 3); it never selects Quick. The others pin it. `quick`/`deep` CLI args override this. | `auto` is least-surprising — full coverage by default, more of it for a large tree, and Quick only when the user asks for it. |
